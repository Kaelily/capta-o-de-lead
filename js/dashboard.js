...
export class DashboardController {
  ...
  bindEvents() {
    if (this.btnNavDashboard) {
      this.btnNavDashboard.addEventListener('click', () => this.showDashboard());
    }
  ...
  renderMetrics() {
    ...
    this.metricTotalLeads.innerText = total;
    this.metricHotLeads.innerText = hotLeads;
    this.metricAvgScore.innerText = `${avgScore}%`;
    this.metricTotalLoss.innerText = `${hotLeads > 0 ? 'R$ 145k+' : 'R$ 0'}`;
  }
  ...
  renderLeadsTable() {
    ...
    leads.forEach(lead => {
      ...
      const waUrl = StorageManager.getWhatsAppLink(lead);
    ...
    if (this.qrModal) {
      if (show) {
        ...
        if (this.inputQRUrl && !this.inputQRUrl.value) {
          this.inputQRUrl.value = defaultUrl;
        }
        this.updateQRCodeImage(this.inputQRUrl.value || defaultUrl);
      } else {
  ...
  updateQRCodeImage(url) {
    if (!url) return;
    if (this.qrCodeImg) {
      // Generate standard scannable QR Code image via QRServer API & Quick Fallback
